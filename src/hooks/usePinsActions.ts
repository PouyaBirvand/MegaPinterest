...
function mergePinsWithoutDuplicates(
  existingPins: Pin[],
  newPins: Pin[]
): Pin[] {
...
export function usePinsActions() {
  ...
  const hidePin = (pinId: string, reason?: string, feedback?: string) => {
    try {
      ...
      if (reason || feedback) {
        const hiddenPinDetails = JSON.parse(
        ...
        localStorage.setItem(
          'hiddenPinDetails',
          JSON.stringify(updatedHiddenDetails)
        );
      }
  ...
  const getHiddenPinDetails = (pinId: string) => {
    try {
      const hiddenDetails = JSON.parse(
        localStorage.getItem('hiddenPinDetails') || '[]'
      );
  ...
  const unhidePin = (pinId: string) => {
    try {
      ...
      const updatedHiddenDetails = hiddenDetails.filter(
        (detail: any) => detail.pinId !== pinId
      );
      localStorage.setItem(
        'hiddenPinDetails',
        JSON.stringify(updatedHiddenDetails)
  ...
  const undoReportPin = (pinId: string) => {
    try {
      ...
      const updatedReports = reports.filter(
        (report: ReportedPin) => report.pinId !== pinId
      );
      localStorage.setItem('pinReports', JSON.stringify(updatedReports));
